...
// 꾸미기 시작하기 버튼 클릭 시
$('.dc_complete button').on('click', function () {
  // 스티커, 텍스트 요소가 없을 시
  if (!isDeco) {
    // alert 제공
    $('#alert01').addClass('on');
    setTimeout(() => {
      $('#alert01').removeClass('on');
    }, 2000);
    // 이미지 다운로드
  } else {
    reset();
...
// 각 이미지 클릭 시
$('.dc_back-image--container label').off().on('click touchstart', function (e) {
  // 이벤트 버블링 방지
  e.stopPropagation();
  // reset 호출
  reset();
...
// ====PC, 모바일 공통 기능====
// 각 sticker 클릭 시 실행
$('.dc_sticker--container button').on('mousedown touchstart', function (e) {
  // 이벤트 버블링 방지
  e.stopPropagation();

  if(notImg) {
    $('.dc_deco-zone').css('background-image', `url(../images/bg_img01.jpg`);
  }
  ...
  const moveable = new Moveable(dragEl, {
    ...
    stickerHeight = $('.dc_drag-area.on img').height();
    frame.set("height", `${stickerHeight}px`);
...
function textReset() {
  ...
  $('.dc_text-color--container input[type="radio"]').prop('checked', false);
  $('.dc_text-size--container input[type="radio"]').prop('checked', false);
...
// text color 클릭 시
$('.dc_text-color--container input').off('click touchstart').on('click touchstart', function (e) {
  // 이벤트 버블링 방지
  e.stopPropagation();
...
// text size 클릭 시
$('.dc_text-size--container input').off('click touchstart').on('click touchstart', function (e) {
  // 이벤트 버블링 방지
  e.stopPropagation();
  ...
  // 폰트 크기에 맞춰 변경될 수 있도록 이전에 설정한 width와 height값을 auto로 변경
  thisDrag.width('auto');
